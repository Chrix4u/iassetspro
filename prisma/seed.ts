...
const rolePermissionBundles: Record<string, string[]> = {
  ...
  plant_manager: [
    ...
    'inventory_adjustments.view', 'inventory_adjustments.update',
...
async function seed() {
  ...
  const systemModules = [
    ...
    { code: 'work_orders', name: 'Work Orders', description: 'Plan, assign, execute, and track corrective repair work orders with SLA management', isCore: true, version: '2.0.0', licensed: true },
    { code: 'repairs', name: 'Repairs Maintenance', description: 'Corrective and emergency repair execution, material/tool requests, transfers, downtime, and closure', isCore: false, version: '2.0.0', licensed: true },
    ...
    { code: 'pm_schedules', name: 'Preventive Maintenance (PM)', description: 'Preventive maintenance schedules, templates, triggers, calendar, and PM work generation', isCore: false, version: '2.0.0', licensed: true },
    ...
    for (const a of assetsData) {
      const asset = await db.asset.create({
        data: {
          name: a.name,
          assetTag: a.assetTag,
          category: { connect: { id: assetCategories[a.category].id } },
