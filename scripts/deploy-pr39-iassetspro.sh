...
TARGET_SHA="1fc128e8c769573b43f730c52d36e1e87e0bc4b8"
...
wait_health() {
  local url="$1" attempts="${2:-18}" label="${3:-health}" code=""
...
[[ -L "$LIVE" ]] || { echo "STOP: $LIVE must be the production symlink." >&2; exit 1; }
...
[[ -f "$OLD_REAL/.next/standalone/server.js" ]] || { echo "STOP: current standalone server is missing." >&2; exit 1; }
OLD_SHA="$(git -C "$OLD_REAL" rev-parse HEAD 2>/dev/null || true)"

echo "============================================================"
echo " iAssetsPro — PR #39 PRODUCTION DEPLOYMENT"
echo "============================================================"
...
say "2/10" "Verify GitHub main is exactly PR #39 merge SHA"
...
say "4/10" "Attach persistent uploads"
...
echo "Canonical path: /home/lightworld/webapps/iassetspro -> $FINAL_REAL"
...
echo "============================================================"
echo " iAssetsPro PR #39 DEPLOYMENT COMPLETE"
echo "============================================================"
echo "$TARGET_SHA"
