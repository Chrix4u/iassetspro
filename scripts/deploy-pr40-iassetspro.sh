...
TARGET_SHA="11f8d257052bad2d45b81500b0d1457311767778"
...
echo "============================================================"
echo " iAssetsPro — PR #40 PRODUCTION DEPLOYMENT"
echo "============================================================"
...
wait_health "http://127.0.0.1:${PROD_PORT}/api/health" 3 "local production" || { echo "STOP: current local production is not healthy." >&2; exit 1; }
wait_health "${PUBLIC_URL}/api/health" 3 "public production" || { echo "STOP: current public production is not healthy." >&2; exit 1; }
pm2 describe "$PM2_NAME" >/dev/null 2>&1 || { echo "STOP: root PM2 process $PM2_NAME not found." >&2; exit 1; }

say "2/10" "Verify GitHub main is exactly merged PR #40 SHA"
...
[[ "$REMOTE_MAIN" == "$TARGET_SHA" ]] || { echo "STOP: main moved. Expected $TARGET_SHA but found $REMOTE_MAIN" >&2; exit 1; }
...
say "4/10" "Keep uploaded files on persistent shared storage"
...
if [[ -L "$OLD_REAL/public/uploads" ]]; then
  ...
  if [[ -d "$EXISTING_UPLOADS" && "$EXISTING_UPLOADS" != "$SHARED_UPLOADS" ]]; then
    if command -v rsync >/dev/null 2>&1; then rsync -a "$EXISTING_UPLOADS/" "$SHARED_UPLOADS/"; else cp -a "$EXISTING_UPLOADS/." "$SHARED_UPLOADS/"; fi
  fi
elif [[ -d "$OLD_REAL/public/uploads" ]]; then
  if command -v rsync >/dev/null 2>&1; then rsync -a "$OLD_REAL/public/uploads/" "$SHARED_UPLOADS/"; else cp -a "$OLD_REAL/public/uploads/." "$SHARED_UPLOADS/"; fi
fi
...
say "5/10" "Validate and build exact merged main release as lightworld"
...
for p in $(seq 3100 3199); do
  if ! ss -ltn | grep -q ":${p} "; then CANARY_PORT="$p"; break; fi
done
[[ -n "$CANARY_PORT" ]] || { echo "STOP: no free canary port found." >&2; exit 1; }
pm2 delete "$CANARY_NAME" >/dev/null 2>&1 || true
PORT="$CANARY_PORT" HOSTNAME=127.0.0.1 NODE_ENV=production pm2 start "$RELEASE/.next/standalone/server.js" --name "$CANARY_NAME" --cwd "$RELEASE" --node-args="--env-file=$RELEASE/.env" --time >/dev/null
...
rollback() {
  ...
  PORT="$PROD_PORT" HOSTNAME=127.0.0.1 NODE_ENV=production pm2 start "$OLD_REAL/.next/standalone/server.js" --name "$PM2_NAME" --cwd "$OLD_REAL" --node-args="--env-file=$OLD_REAL/.env" --time >/dev/null
  wait_health "http://127.0.0.1:${PROD_PORT}/api/health" 18 "rollback" || { pm2 logs "$PM2_NAME" --lines 150 --nostream || true; echo "ROLLBACK FAILED: manual intervention required." >&2; return 1; }
  pm2 save >/dev/null
...
if [[ "$PORT_CLEARED" != "1" ]]; then echo "Production port $PROD_PORT did not clear." >&2; rollback || true; exit 1; fi
PORT="$PROD_PORT" HOSTNAME=127.0.0.1 NODE_ENV=production pm2 start "$RELEASE/.next/standalone/server.js" --name "$PM2_NAME" --cwd "$RELEASE" --node-args="--env-file=$RELEASE/.env" --time >/dev/null
if ! wait_health "http://127.0.0.1:${PROD_PORT}/api/health" 18 "new production"; then pm2 logs "$PM2_NAME" --lines 150 --nostream || true; rollback; exit 1; fi

say "8/10" "Atomically point canonical iAssetsPro path at healthy release"
rm -f "${LIVE}.next"
ln -s "$RELEASE" "${LIVE}.next"
mv -Tf "${LIVE}.next" "$LIVE"
[[ "$(readlink -f "$LIVE")" == "$RELEASE" ]] || { echo "Canonical symlink promotion failed." >&2; rollback; exit 1; }

say "9/10" "Public smoke and login-production sanity checks"
if ! wait_health "${PUBLIC_URL}/api/health" 12 "public"; then rollback; exit 1; fi
ROOT_HTML="$(curl -fsS --max-time 20 "${PUBLIC_URL}/" || true)"
if [[ -n "$ROOT_HTML" ]]; then
  for forbidden in 'Demo Accounts' 'admin123' 'password123' '99.9%' '24/7' '27001 Certified' 'iAssetsPro-WO-Workflow-Presentation.pptx' 'enterprise-grade encryption'; do
    if grep -Fq "$forbidden" <<<"$ROOT_HTML"; then echo "STOP: forbidden production-login text still present: $forbidden" >&2; rollback; exit 1; fi
  done
  echo "Public health and login-production sanity checks: PASS"
else
...
echo "============================================================"
echo " iAssetsPro PR #40 DEPLOYMENT COMPLETE"
echo "============================================================"
echo "$TARGET_SHA"
