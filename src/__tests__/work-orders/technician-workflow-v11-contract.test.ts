...
describe('technician workflow V1.1 completion contract', () => {
  ...
  it('prefetches available store materials, tools and units instead of free-text resource identities', () => {
    ...
    expect(panel).toContain('api.get<{ availableTools: ToolOption[]; recommendedTools: any[] }>(');
    ...
    expect(panel).toContain(".filter((tool: ToolOption) => tool.status === 'available' && Number(tool.quantity ?? 1) > 0)");
