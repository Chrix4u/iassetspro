...
export async function GET(request: NextRequest) {
  try {
    ...
    // Every operational domain used by the dashboard is license-gated.
    // Admin bypasses user permissions only; admin never bypasses a disabled,
    // inactive, expired, or unlicensed module.
    ...
    const canViewWorkOrderKPIs = moduleOperational('work_orders')
      && (isAdm || hasAnyPermission(session, ['work_orders.view', 'work_orders.view_all', 'work_orders.view_own']));
    const canViewRequestKPIs = moduleOperational('maintenance_requests')
      && (isAdm || hasAnyPermission(session, ['maintenance_requests.view', 'maintenance_requests.view_all', 'maintenance_requests.view_own']));
    ...
    // Disabled/unlicensed WO/MR modules receive impossible scopes so downstream
    // queries fail closed even if a future card forgets its display guard.
