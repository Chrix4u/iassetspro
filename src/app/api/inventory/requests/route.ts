...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isAdmin(session) && !hasAnyPermission(session, ['material_requisitions.view', 'material_requisitions.create', 'material_requisitions.update', 'material_requisitions.approve', 'material_requisitions.issue', 'material_requisitions.reject'])) {
      return NextResponse.json({ success: false, error: 'Insufficient material requisition permissions' }, { status: 403 });
    }
...
export async function POST(request: NextRequest) {
  try {
    const session = getSession(request);
    if (!session) return NextResponse.json({ success: false, error: 'Not authenticated' }, { status: 401 });
    if (!hasPermission(session, 'material_requisitions.create') && !isAdmin(session)) {
