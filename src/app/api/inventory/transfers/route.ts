...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!canViewInventoryArea(session, 'inventory_transfers.view')) {
      return NextResponse.json({ success: false, error: 'Insufficient permissions to view inventory transfers' }, { status: 403 });
    }
