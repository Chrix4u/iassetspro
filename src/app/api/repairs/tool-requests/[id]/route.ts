    ...
    const cancellation = await db.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityId: id,
          newValues: JSON.stringify({ toolName: toolReq.toolName, workOrderId: toolReq.workOrderId }),
        },
      });
      return true;
    });
    if (!cancellation) {
      return NextResponse.json({ success: false, error: 'Tool request changed concurrently and can no longer be cancelled' }, { status: 409 });
    }
