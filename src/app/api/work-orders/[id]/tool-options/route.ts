 ...
 *
 * Least-privilege selector for one work order. Exact execution membership is a
 * relationship-scoped authority boundary: the endpoint never exposes another
 * plant or the global Tool Registry. Non-execution management callers still
 * require strict plant authorization.
 */
  ...
  try {
    ...
    if (isExecutionActor) {
      ...
      if (!canManageWorkOrder(session, wo)) {
        return NextResponse.json(
          { success: false, error: 'Tool options require assigned execution access or accountable maintenance-management authority' },
          { status: 403 },
        );
      }
    }
    ...
    const [availableTools, recommendedRecords] = await Promise.all([
      ...
      recommendedIds.length > 0
        ? db.tool.findMany({
            where: { id: { in: recommendedIds }, plantId: wo.plantId },
            select: {
              id: true,
              toolCode: true,
              name: true,
    ...
    return NextResponse.json({
      success: true,
      data: { availableTools, recommendedTools, plantId: wo.plantId },
    });
  } catch (error: unknown) {
