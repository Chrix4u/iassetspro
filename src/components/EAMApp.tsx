...
import { getPageAccessRule, hasPagePermission, isPageModuleAvailable } from '@/lib/pageAccess';
...
function PageSwitcher({ page }: { page: string }) {
  ...
  const admin = isAdmin();
  const accessRule = getPageAccessRule(page);
  const permissionAllowed = hasPagePermission(page, hasPermission, admin);
  const moduleAllowed = isPageModuleAvailable(page, enabledModules);
  const moduleStatePending = enabledModules === null && (accessRule.moduleCode || 'core') !== 'core';

  // Route guard: permissions and module/license state must both allow the page.
  useEffect(() => {
    if (!permissionAllowed) {
      navigate('dashboard');
      return;
    }
    if (!moduleStatePending && !moduleAllowed) {
      navigate('dashboard');
    }
  }, [permissionAllowed, moduleAllowed, moduleStatePending, navigate]);
  ...
  }, [page, permissionAllowed, moduleAllowed]);
  ...
  if (!permissionAllowed || !moduleAllowed || !Component) return <LoadingSkeleton />;
  return <Component />;
}
