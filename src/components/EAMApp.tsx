...
function PageSwitcher({ page }: { page: string }) {
  ...
  const modulesLoaded = useNavigationStore((s) => s.modulesLoaded);

  const accessAllowed = modulesLoaded && canAccessPage(page as PageName, {
    permissions,
    isAdmin: isAdmin(),
    enabledModules,
  });

  // Permission + licensing guard: do not load hidden/unlicensed pages.
  useEffect(() => {
    if (!modulesLoaded) return;
    if (!canAccessPage(page as PageName, {
      permissions,
      isAdmin: isAdmin(),
      enabledModules,
    })) {
  ...
  }, [page, permissions, isAdmin, enabledModules, modulesLoaded, navigate]);

  useEffect(() => {
    if (!modulesLoaded || !accessAllowed) {
      setComponent(null);
      setError(null);
      return;
    }
  ...
  }, [page, modulesLoaded, accessAllowed]);

  if (!modulesLoaded || !accessAllowed) return <LoadingSkeleton />;
