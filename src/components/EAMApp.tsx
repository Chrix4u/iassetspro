...
import { PAGE_MODULE_REQUIREMENTS, hasPagePermission, isPageModuleAvailable } from '@/lib/page-access';
...
const PAGE_TITLES: Record<string, string> = {
  ...
  // Maintenance
  'maintenance-work-orders': 'Repair Work Orders',
  'wo-detail': 'Work Order Details',
  'maintenance-requests': 'Repair Requests',
  'mr-detail': 'Request Details',
  'create-mr': 'New Request',
  'maintenance-dashboard': 'Repairs Dashboard',
  'maintenance-analytics': 'Work Order Analytics',
...
function PageSwitcher({ page }: { page: string }) {
  ...
  const pageName = page as PageName;
  const moduleStatePending = Boolean(PAGE_MODULE_REQUIREMENTS[pageName]) && enabledModules === null;
  const permissionAllowed = hasPagePermission(pageName, hasPermission, isAdmin());
  const moduleAllowed = !moduleStatePending && isPageModuleAvailable(pageName, enabledModules);
  const pageAllowed = permissionAllowed && moduleAllowed;

  // Enforce one shared access contract for sidebar, direct navigation and page loading.
  // Licensed-module pages stay blank while module state is loading, then redirect
  // if the module is disabled/unlicensed or the user lacks page permission.
  useEffect(() => {
    if (moduleStatePending) {
      setComponent(null);
      return;
    }
    if (!permissionAllowed || !moduleAllowed) {
      setComponent(null);
      navigate('dashboard', {}, true);
    }
  }, [moduleStatePending, permissionAllowed, moduleAllowed, navigate]);
  ...
  if (moduleStatePending || !pageAllowed || !Component) return <LoadingSkeleton />;
  return <Component />;
}
