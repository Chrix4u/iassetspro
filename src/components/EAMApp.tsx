...
function PageSwitcher({ page }: { page: string }) {
  ...
  // Centralized page policy is shared by sidebar, mobile navigation and direct routing.
  const accessResolved = enabledModules !== null;
  const pageAllowed = accessResolved && canAccessPage(page, { hasPermission, isAdmin, enabledModules });

  // Direct navigation is guarded by both RBAC and module licensing/activation.
  useEffect(() => {
    if (!accessResolved) return;
    if (!pageAllowed) navigate('dashboard', {}, true);
  }, [accessResolved, pageAllowed, navigate]);

  useEffect(() => {
    if (!accessResolved || !pageAllowed) {
      setComponent(null);
      setError(null);
      return;
    }
  ...
  }, [page, accessResolved, pageAllowed]);

  if (!accessResolved || !pageAllowed) return <LoadingSkeleton />;
