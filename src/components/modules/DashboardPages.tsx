...
export function DashboardPage() {
  ...
  const { navigate, enabledModules: storeEnabledModules, fetchModules } = useNavigationStore();
  ...
  // Reuse the central licensed/enabled module state. Optional modules fail
  // closed while this state is loading or unavailable.
  useEffect(() => {
    fetchModules();
  }, [fetchModules]);
  ...
  const visibleQuickActions = allQuickActions
    ...
    .filter(a => !['pm-schedules', 'pm-templates', 'pm-triggers', 'pm-calendar'].includes(a.page) || enabledModules.has(MODULE_CODES.PM_SCHEDULES));
  ...
  // Module-aware visibility is explicit: disabled/unlicensed modules never
  // contribute dashboard cards, shortcuts, or drill-down links.
