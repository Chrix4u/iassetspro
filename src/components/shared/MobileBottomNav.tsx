...
export function MobileBottomNav({ onMenuOpen }: MobileBottomNavProps) {
  ...
  // Use the same centralized permission + module/license policy as desktop and the router.
  const visibleTabs = useMemo(() => {
    const admin = isAdmin();
    return BOTTOM_TABS.filter((tab) =>
      canAccessPage(tab.page, hasPermission, admin, enabledModules),
    );
  ...
  const visibleMoreItems = useMemo(() => {
    ...
    return MORE_ITEMS.filter((item) =>
      canAccessPage(item.page, hasPermission, admin, enabledModules),
    );
  }, [hasPermission, isAdmin, enabledModules]);
