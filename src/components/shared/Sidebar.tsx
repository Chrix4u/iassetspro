...
function SidebarContent({ forceExpanded }: { forceExpanded?: boolean } = {}) {
  ...
  // Module state is authoritative and fail-closed. Business modules are never
  // exposed while licensing/activation state is unknown.
  ...
  const menuGroups = useMemo<NavGroup[]>(() => [
    ...
    {
      label: 'Repairs Maintenance', icon: Wrench, perm: 'work_orders.view', moduleCodes: ['work_orders', 'maintenance_requests', 'repairs'],
      children: [
        ...
        { page: 'maintenance-dashboard', label: 'Repairs Dashboard', icon: LayoutDashboard },
        ...
        { page: 'technician-timesheet', label: 'Technician Timesheet', icon: Clock },
  ...
  // Resolve visibility from the same page-level policy used by direct navigation.
  // A parent group only exists when at least one child is both permitted and licensed.
  ...
  }, [menuGroups, permissions, isAdmin, enabledModules]);
