...
export async function apiFetch<T = any>(
  ...
  // Never send an online protected request once the bearer token has
  // disappeared from storage. Keep actor-bound offline work queueable, but
  // synchronize real session expiry with the UI before a 401/403 cascade.
  if (
  ...
  try {
    ...
    if (!res.ok || payload.success === false) {
      ...
      // worker handoff, a still-valid persisted bearer may be rejected once.
      // Retry exactly once before clearing local auth. Mutations are never
      // replayed here.
